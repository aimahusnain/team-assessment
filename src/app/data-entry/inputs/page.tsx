                ...
                <TableCell>
                  <div className="flex items-center gap-1">
                    <Input
                      ...
                      onChange={(e) => onUpdate?.(`${score}_benchmark`, e.target.value)}
                      className="w-32"
                    />
                ...
                <TableCell>
                  <div className="flex items-center gap-1">
                    <Input
                      ...
                      onChange={(e) => onUpdate?.(`${score}_interval`, e.target.value)}
                      className="w-32"
                    />
                    {score === "ce" || score === "rbsl" ? <span>%</span> : null}
                  </div>
...
export default function Inputs() {
    ...
    try {
      ...
      if (data.success && data.data.length > 0) {
        ...
        setConfig({
          ...
          individual_score_tcm_benchmark: Number(inputData.individual_score_tcm_benchmark),
          individual_score_tcm_interval: Number(inputData.individual_score_tcm_interval),
          individual_score_ce_benchmark: String(inputData.individual_score_ce_benchmark),
          individual_score_ce_interval: String(inputData.individual_score_ce_interval),
          individual_score_ts_benchmark: Number(inputData.individual_score_ts_benchmark),
          individual_score_ts_interval: Number(inputData.individual_score_ts_interval),
          individual_score_rbsl_benchmark: String(inputData.individual_score_rbsl_benchmark),
          individual_score_rbsl_interval: String(inputData.individual_score_rbsl_interval),
          ...
          team_score_rbsl_benchmark: String(inputData.team_score_rbsl_benchmark),
  ...
  const handleWeightChange = (field: keyof Config["weights"], value: number) => {
    if (config) {
      setConfig({
        ...config,
  ...
  const handleScoreUpdate = (type: "individual" | "team", field: string, value: string | number) => {
    if (config) {
      setConfig({
        ...config,
  ...
  const totalWeight = config ? Object.values(config.weights).reduce((a, b) => a + Number(b), 0) : 0
    ...
    try {
      const res = await fetch("/api/update-inputs", {
        ...
        body: JSON.stringify({
          ...
          individual_score_rbsl_benchmark: config.individual_score_rbsl_benchmark,
        ...
        <div className="grid gap-6">
          <div className="flex sm:flex-row flex-col gap-5 sm:items-center items-start justify-between">
            ...
            <div className="flex space-x-2">
              ...
              <Button onClick={saveConfig} disabled={!validateWeights() || saving || loading}>
          ...
          {loading ? (
            ...
            <div className="grid gap-6">
              <Card className="shadow-sm">
                <CardHeader>
                  <CardTitle className="text-lg font-semibold text-primary">Threshold Settings</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <Label htmlFor="callMin">Total Call Minutes Threshold</Label>
                      <Input
                        id="callMin"
                        type="number"
                        value={config.totalCallMinThreshold}
                        onChange={(e) => handleInputChange("totalCallMinThreshold", Number(e.target.value))}
                      />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="teamMembers">Team Members Threshold</Label>
                      <Input
                        id="teamMembers"
                        type="text"
                        value={config.teamMemberThreshold}
                        onChange={(e) => handleInputChange("teamMemberThreshold", e.target.value)}
                      />
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="shadow-sm">
                <CardHeader>
                  <CardTitle className="text-lg font-semibold text-primary">Weight Distribution</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-sm font-bold ${validateWeights() ? "text-green-500" : "text-red-500"}`}>
                          {totalWeight}%
                        </span>
                      </div>
                    </div>
                      ...
                      {Object.entries(config.weights).map(([key, value]) => (
                        <div key={key} className="space-y-2">
                          ...
                          <Slider
                            ...
                            max={100}
                            step={1}
                            onValueChange={(vals) => handleWeightChange(key as keyof Config["weights"], vals[0])}
                          />
                        </div>
                      ))}
                    </div>
                ...
                <ScoreTable
                  ...
                  data={Object.fromEntries(
                    Object.entries(config)
                      ...
                      .map(([key, value]) => [key.replace("individual_score_", ""), value]),
                  )}
                  onUpdate={(field, value) => handleScoreUpdate("individual", field, value)}
                />
                <ScoreTable
                  ...
                  data={Object.fromEntries(
                    Object.entries(config)
                      ...
                      .map(([key, value]) => [key.replace("team_score_", ""), value]),
                  )}
                  onUpdate={(field, value) => handleScoreUpdate("team", field, value)}
                />
              </div>
            </div>
          ) : (
